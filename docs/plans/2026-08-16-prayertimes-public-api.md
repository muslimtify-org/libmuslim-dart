...
- [x] Step 1: Replace the entire contents of `lib/prayertimes.dart` with:
```dart
...
- [x] Step 2: Delete `test/prayertimes_test.dart`

- [x] Step 3: In `example/lib/main.dart`, replace everything above the line `class MyApp extends StatefulWidget {` with:
```dart
...
- [x] Step 4: In `example/lib/main.dart`, change the field declaration `late Map<String, String> times;` to:
```dart
  late PrayerTimes times;
```

- [x] Step 5: In `example/lib/main.dart`, replace the two lines
```dart
...
- [x] Step 6: In `example/pubspec.yaml`, delete the line `  ffi: ^2.1.4` and the blank line following it from the `dependencies` block

- [x] Step 7: In `README.md`, replace the fenced `dart` block under `## Usage` and the paragraph immediately after it (the one beginning "These are the raw generated bindings") with:
````markdown
```dart
import 'package:libmuslim_dart/prayertimes.dart';
...
- [x] Step 8: Run `flutter pub get` inside `example/`
- [x] Step 9: Run `dart test`
- [x] Step 10: Run `dart analyze`
- [x] Step 11: Run `flutter test` inside `example/`
- [x] Step 12: Commit

**Result:** `117d693`. Every part of the clause passed, re-run independently: both greps report no match (exit 1), `dart test` exit 0 with 32 tests, `dart analyze` exit 0, and `flutter test` in `example/` exit 0 with the widget test unmodified.

Scope carried one file beyond the list: `example/pubspec.lock`, regenerated by the `flutter pub get` that Step 8 commands. Its whole diff is `ffi` moving from `direct main` to `transitive` — the mechanical proof that the example no longer depends on `package:ffi`, which is what spec Goal 1 asked for. The root `pubspec.lock` is untracked, so Task 1's `dart pub get` left nothing behind.
